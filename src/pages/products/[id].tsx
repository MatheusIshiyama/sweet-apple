import { Box, Stack, Text, Image, Flex, Button, Heading, SimpleGrid, StackDivider, useColorModeValue, VStack } from '@chakra-ui/react';
...
export default function ProductDetailsPage({ product: productData }: { product: Product }) {
  const { name, image, price, rating, description, isAvailable } = productData;
  ...
  useEffect(() => {
    document.title = `Sweet Apple | ${name}`;
  });
  ...
  return (
    <Box px={getDefaultPadding('x')} py={getDefaultPadding('y')} bg={GetBackgroundColor()}>
      <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={{ base: 8, md: 10 }} py={{ base: 18, md: 24 }}>
        <Flex>
          <Image rounded="md" alt={name} src={image} fit="cover" align="center" w="100%" h={{ base: '100%', sm: '400px', lg: '500px' }} />
        </Flex>
        <Stack spacing={{ base: 6, md: 10 }}>
          ...
          >
            <Text color={useColorModeValue('gray.500', 'gray.400')} fontSize="2xl" fontWeight="300" h={{ base: 'auto', lg: '215px' }}>
              {description}
            </Text>
          </Stack>

          {isAvailable ? (
            <Button variant="primaryButton" width="full" onClick={() => addProductToCart(productData)}>
              Add to cart
            </Button>
          ) : (
